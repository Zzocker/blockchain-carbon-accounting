...
export class EmissionsRecordContract {
   ...
   */
  async recordEmissions(utilityId:string, partyId:string, fromDate:string, thruDate:string, energyUseAmount:string, energyUseUom:string, url:string, md5:string){
      ...
      const factor = await this.utilityEmissionsFactorState.getEmissionsFactorByLookupItem(lookup.item,thruDate);
      const co2Emission = getCO2EmissionFactor(factor.factor,Number(energyUseAmount),energyUseUom);
