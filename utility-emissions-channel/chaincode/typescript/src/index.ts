...
class EmissionsChaincode {
  ...
  async recordEmissions(stub:ChaincodeStub,args:string[]):Promise<ChaincodeResponse>{
    ...
    const fields = ['utilityId', 'partyId', 'fromDate', 'thruDate', 'energyUseAmount', 'energyUseUom', 'url', 'md5'];
    const fieldsMap = {
      utilityId: null,
      partyId: null,
      fromDate: null,
    ...
    let byte:Uint8Array;
    try {
        byte = await (new EmissionsRecordContract(stub)).recordEmissions(fieldsMap.utilityId,
          fieldsMap.partyId,
          fieldsMap.fromDate,
          fieldsMap.thruDate,
    ...
    logger.debug(
      `${MsgSuccess} recordEmissions success ${byte.toString()}`,
    );
  ...
  async getAllEmissionsDataByDateRange(stub:ChaincodeStub,args:string[]):Promise<ChaincodeResponse>{
    ...
    let byte:Uint8Array;
    try {
      byte = await (new EmissionsRecordContract(stub)).getAllEmissionsDataByDateRange(args[0],args[1]);
    } catch (error) {
      logger.error(error);
      return Shim.error(stringToBytes((error as Error).message));
    }
  ...
  async getAllEmissionsDataByDateRangeAndParty(stub:ChaincodeStub,args:string[]):Promise<ChaincodeResponse>{
    ...
    let byte:Uint8Array;
    try {
      byte = await (new EmissionsRecordContract(stub)).getAllEmissionsDataByDateRangeAndParty(args[0],args[1],args[2]);
    } catch (error) {
      logger.error(error);
      return Shim.error(stringToBytes((error as Error).message));
    }
